    ...
    <AuthContext.Provider
      value={{
        signIn: async (datas) => {
          ...
          setIsLoading(true);
          try {
            const response = await fetch(`https://tranzaxx.com/api/auth/login`, {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
                // 'Accept': 'application/json'
              },
              body: JSON.stringify({ login: datas.email, password: datas.password, captcha_key: datas.captcha_key }),
            })
            ...
            if (data.success) {
              ...
              saveToken('user', JSON.stringify({ ...data.result, isAdmin: data.extra.isAdmin }))
              return true;
            }
